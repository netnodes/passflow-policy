---
go_back: true
---

# PassFlow Terms of Service

These Terms of Service (“Terms”) govern your access to and use of the PassFlow platform, APIs, dashboard, and related services (“Service”). PassFlow is a trading name of **NetNodes Limited** (“NetNodes”, “we”, “us”).

By creating an account, accessing, or using the Service, you agree to be bound by these Terms.

If you are entering into these Terms on behalf of an organisation, you represent that you have authority to bind that organisation. In such cases, “Customer” refers to that organisation.

---

## 1. Accounts & Eligibility

1.1 **Minimum Age.** You must be at least 16 years old or have a lawful basis to process data for individuals under 16.  
1.2 **Accurate Information.** You must provide accurate account information and keep it updated.  
1.3 **Account Security.** You are responsible for safeguarding your login credentials.  
1.4 **Authorised Users.** Each user must have their own login. Sharing accounts is not permitted.  
1.5 **Acceptable Use.** You may not use the Service for any unlawful, fraudulent, or abusive activity. This includes creating misleading passes, phishing, impersonation, or bulk spam distribution.

---

## 2. Data Protection & Privacy

2.1 **Customer Data Ownership.** Customer retains all rights to the data, assets, and content uploaded or created within the Service (“Customer Data”).  
2.2 **Processor Role.** To the extent NetNodes processes personal data on behalf of the Customer, NetNodes acts as a **Data Processor**.  
2.3 **DPA Incorporation.**  
By creating an account or using the Service, the Customer automatically enters into the **Data Processing Agreement (DPA)** available at https://policy.netnodes.net/dpa which forms part of these Terms.  
2.4 **Privacy Policy.** Use of the Service is also subject to our Privacy Policy at https://policy.passflow.io/privacy

---

## 3. Use of the Service

3.1 **Licence to Use.** We grant you a limited, non-exclusive, non-transferable right to use the Service.  
3.2 **Restrictions.** You may not:  
- copy, modify, or reverse engineer the Service  
- interfere with the Service or its security features  
- use the Service to send unsolicited messages  
- bypass API limits or create automated systems that abuse capacity  
- use the platform for illegal or harmful purposes  
3.3 **Third-Party Platforms.** PassFlow relies on Apple Wallet, Google Wallet, and other third-party services. Their terms also apply.

---

## 4. API Terms

4.1 **API Access.** Customers may access the Service via documented APIs.  
4.2 **Rate Limits.** Excessive or abusive API usage may result in temporary or permanent suspension.  
4.3 **No Warranty for Integrations.** We are not responsible for third-party integrations using our API.

---

## 5. Subscriptions, Billing & Taxes

5.1 **Billing.** Paid plans require a valid payment method. Services are billed in advance.  
5.2 **Refunds.** Subscription fees are non-refundable.  
5.3 **Plan Changes.** Upgrades take effect immediately; downgrades take effect in the next billing cycle.  
5.4 **Taxes.** Fees exclude applicable taxes, which will be added where required.

---

## 6. Cancellation & Termination

6.1 **Customer Cancellation.** You may cancel your account at any time via your dashboard.  
6.2 **Effect of Cancellation.** Upon cancellation, Customer Data may be deleted after a retention period as described in our DPA and Privacy Policy.  
6.3 **Termination by NetNodes.** We may suspend or terminate access where:  
- terms are violated  
- required by law  
- payment is overdue  
6.4 **Survival.** Sections relating to IP, liability, indemnities, and data protection survive termination.

---

## 7. Intellectual Property

7.1 **NetNodes IP.** All rights to the Service, software, documentation, and underlying technology belong to NetNodes.  
7.2 **Customer IP.** Customer retains ownership of Customer Data and materials uploaded.

---

## 8. Security

8.1 **Our Security.** We implement industry-standard technical and organisational measures to protect Customer Data.  
8.2 **Customer Responsibility.** You are responsible for appropriate configuration, access control, and managing your users.  
8.3 **Physical Security.** PassFlow does not assume responsibility for physical security outcomes, access decisions, or site security.

---

## 9. Availability & Support

9.1 **Availability.** We aim for high availability but do not guarantee uninterrupted service.  
9.2 **Support.** Email support is available for paid plans.

---

## 10. Limitation of Liability

To the maximum extent permitted by law:

- NetNodes’ total liability arising from these Terms or use of the Service will not exceed the **fees paid by the Customer in the 12 months preceding the claim**.  
- NetNodes is not liable for indirect, incidental, consequential, special, or punitive damages, including lost profits, revenue, data, or business interruption.  

Nothing in this clause limits liability that cannot be excluded by law.

---

## 11. Indemnification

Customer will indemnify and hold NetNodes harmless from claims arising from:  
- misuse of the Service  
- unlawful or misleading digital passes  
- infringement caused by Customer content  
- breach of these Terms

---

## 12. Modifications

We may update these Terms by posting revised terms on passflow.io. Continued use of the Service constitutes acceptance.

---

## 13. Governing Law

These Terms are governed by the laws of England and Wales.  
For EU/EEA customers, mandatory consumer or data protection rights continue to apply.

---

## 14. Contact

Questions regarding these Terms may be sent to: **support@netnodes.net**

